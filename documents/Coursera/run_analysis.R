## This file will read datasets for testing and training data (provided they are in your working directory), merge the two sets into a single dataset, add desriptive names to the variables and activities, extracts only the the mean values and standard deviation measurements,and finally, create a new data frame consisting of the mean value of every measurement, for each activity for all subjects.


#required packageslibrary(dplyr)
...
ALLD <- rbind(test,train)                                                   #create large data frame merging test and train datanames
